...
# Imágenes de análisis: leídas desde esta carpeta (no por multipart).
BENCHMARK_IMAGES_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "benchmark viajes pallet",
    "grjr57_2_101126_mediodía",
    "0e2e25b8-2f56-4e56-9a09-e35dadaa4b74",
)
_BENCHMARK_IMG_EXTS = {".jpg", ".jpeg", ".png", ".webp", ".bmp"}


def listar_imagenes_benchmark():
    """Rutas absolutas de imágenes en BENCHMARK_IMAGES_DIR, ordenadas por nombre."""
    if not os.path.isdir(BENCHMARK_IMAGES_DIR):
        return []
    salida = []
    for nombre in sorted(os.listdir(BENCHMARK_IMAGES_DIR)):
        ruta = os.path.join(BENCHMARK_IMAGES_DIR, nombre)
        if os.path.isfile(ruta) and os.path.splitext(nombre)[1].lower() in _BENCHMARK_IMG_EXTS:
            salida.append(ruta)
    return salida
...
def ejecutar_analisis(
    ...
    """
    Ejecuta el pipeline sobre las imágenes en BENCHMARK_IMAGES_DIR.
    ...
    rutas_origen = listar_imagenes_benchmark()
    if not rutas_origen:
        raise FileNotFoundError(f"No hay imágenes en: {BENCHMARK_IMAGES_DIR}")
    ...
    max_post = ""
    validos = []
    for idx, ruta_origen in enumerate(rutas_origen[:3]):
    ...
    if not max_post:
        raise RuntimeError(
            "No se pudo leer ninguna imagen válida desde la carpeta benchmark."
        )
    ...
    if (len(validos) < 3) or True:
        ...
        if len(groups[0]) == len(groups[1]) == len(groups[2]):
            ...
            for i, g in enumerate(groups[0]):
                ...
                draw.text((int(g[2]), int(g[1])), clase_predicha, fill=(0, 255, 0))
        ...
        cuenta_colores["azul"] = round(l_cuenta_colores["azul"] / 3)
        cuenta_colores["blanco"] = round(l_cuenta_colores["blanco"] / 3)
        cuenta_colores["rojo"] = round(l_cuenta_colores["rojo"] / 3)
    ...
    for arch in glob.glob("salida/*", recursive=True):
